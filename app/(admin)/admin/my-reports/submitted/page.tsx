...
export default function AdminSubmittedReportsPage() {
  const [reports, setReports] = useState<Report[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  // No selection functionality needed for submitted reports
  ...
  const fetchReports = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      // Filter by SUBMITTED status and add pagination
      const response = await fetch(`/api/reports?status=SUBMITTED&page=${page}`);
  ...
  // No selection handlers needed for submitted reports
  ...
  return (
    <div className="p-4 space-y-4 h-full overflow-y-auto">
      <div className="flex flex-col gap-4">
        {/* No selection controls needed for submitted reports */}
